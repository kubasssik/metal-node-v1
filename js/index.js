...
function createAbbreviatedPrice() {
    function rP() {
        ...
        for (let i = 0; i < dataPrice.length; i++) {
            const e = dataPrice[i]
            if (i == 0) continue
            if (e.length == 7 || e.length == 6 || e.length == 5 ||  e.length == 2) {
                aPr.push(e)
            }
        }
        return aPr
    ...
    function creatPrice() {
        for (let i = 0; i < tr.length; i++) {
            for (let j = 1; j < 7; j++) {
                if( j % 6 == 0){
                  if(rP()[i][j-1] == null)   createElement(1, 'td', '__td-red', tr[i], '') 
                     else   createElement(1, 'td', '__td-red', tr[i], rP()[i][j-1].toFixed(2))      
                }
                else createElement(1, 'td', '__td', tr[i], rP()[i][j-1])
            }
        }
    }
    creatPrice()
...
function createFullPrice() {
    ...
    for (let i = 0; i < dataPrice.length; i++) {
        ...
        if (e.length == 7 || e.length == 6 || e.length == 5 ||  e.length == 2) {
            for (let j = 1; j < 7; j++) {
                if( j % 6 == 0){
                     if(e[j-1] == null) createElement(1, 'td', '__td-red', tr[i - 1], '')  
                     else   createElement(1, 'td', '__td-red', tr[i - 1], e[j-1].toFixed(2))    
                }
                else createElement(1, 'td', '__td', tr[i - 1], e[j-1])
            }
    ...
    th.forEach(e => e.setAttribute('colspan', '6'))
}
createFullPrice()
     ...
     const $priceControlBtn = document.querySelectorAll('.price__control_btn'); 
     const $priceDataBlock = document.querySelector('.price__data_block')
     $downloadPriceButton.addEventListener('click', openPrice)
     $priceControlBtn.forEach(e => {
    e.addEventListener('click', openPriceControl)
} )
function openPriceControl() {
